...
# MAGIC 
# MAGIC write.df(df.out, source = "delta", path = "/mnt/completejourney/silver/features_finalized", mode="overwrite", overwriteSchema="true")

# COMMAND ----------

# DBTITLE 1,Retrieve Eigenvalues in Python
display(
  spark.table('DELTA.`/mnt/completejourney/silver/features_finalized/`')
  )
...
# generate correlations between features
famd_features_corr = spark.table('DELTA.`/mnt/completejourney/silver/features_finalized/`').drop('household_id').toPandas().corr()
